try { 
  ...
  #Install-ChocolateyZipPackage '__NAME__' '$(Split-Path -parent $MyInvocation.MyCommand.Definition)' "$(Split-Path -parent $MyInvocation.MyCommand.Definition)"

	$packageFolder = Split-Path -parent $MyInvocation.MyCommand.Definition
	$constructZip = "$packageFolder\..\content\Construct-Build-Scripts.zip"
	Get-ChocolateyUnzip $(Resolve-Path "$constructZip") "$env:USERPROFILE\Documents\WindowsPowerShell\Modules\Construct"
  ...
  #Write-ChocolateySuccess 'Coldwater.Construct.2012'
} catch {
  Write-ChocolateyFailure 'Coldwater.Construct.2012' "$($_.Exception.Message)"
  throw 
}